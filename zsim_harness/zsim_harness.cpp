...
int main(int argc, char *argv[]) {
    ...
    if (attachDebugger) {
	    ...
	    deadlockDetection = conf.get<bool>("sim.deadlockDetection", false);
    }
	...
	for (int i = 0; i < 16; i++) {
		...
		for (int j = 0; j < 16; j++) {
				s += std::to_string(zinfo->hmc.vault[i].PIMNeed[j]) + " ";
			    
			//	info((std::to_string(zinfo->hmc.vault[i].PIMNeed[j])).c_str());
			//info(s);
		}
		d += std::to_string(zinfo->hmc.vault[i].PIMCount) + " ";
		s += "\n";
	}
	info(s.c_str());
	info(d.c_str());
