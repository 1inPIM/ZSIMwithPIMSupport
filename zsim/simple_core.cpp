...
uint64_t SimpleCore::flush(InvReq& req, int level) {
	
	if (level == 0)
		return l1i->flush(req);
	else {
		if (level == 1)
			return l1d->flush(req);
		else
		{
			if (level == 2)
				return l2->flush(req);
...
bool SimpleCore::tryInvalidate(InvReq& req, int level)
{
	if (level == 0)
		return l1i->tryInvalidate(req);
	else {
		if (level == 1)
			return l1d->tryInvalidate(req);
		else
		{
			if (level == 2)
				return l2->tryInvalidate(req);
		}
	}
	return l3->tryInvalidate(req);
...
void SimpleCore::load(Address addr) {
	...
	if (zinfo->switchmode)
	{
		zinfo->record.push_back(std::make_tuple(curCycle,addr,false));
		//zinfo->hmc->access(addr, false);
	}
	//if (!zinfo->switchmode)
	//(zinfo->hmc).messureDistance(addr);
...
void SimpleCore::store(Address addr) {
	if (zinfo->switchmode)
	{
		zinfo->record.push_back(std::make_tuple(curCycle, addr, true));
		//zinfo->hmc->access(addr, true);
		
	}
	//if (!zinfo->switchmode)
	//	zinfo->hmc.messureDistance(addr,false);
...
void SimpleCore::BblFunc(THREADID tid, ADDRINT bblAddr, BblInfo* bblInfo) {
    ...
    while (core->curCycle > core->phaseEndCycle) {
	    ...
	    //	    else
	    //	    {
	    //		    if (core->phaseEndCycle != zinfo->globPhaseCycles + zinfo->phaseLength)
	    //		    {
	    //			    if (zinfo->switchmode)
	    //			    {
	    //				    core->phaseEndCycle = ((SimpleCore*)zinfo->cores[1])->phaseEndCycle;
	    //			    }
	    //			    else
	    //			    {
	    //				    core->phaseEndCycle = ((SimpleCore*)zinfo->cores[0])->phaseEndCycle;
	    //				    
	    //			    }
	    //			    
	    //		    }
	    ...
	    {
		    core->phaseEndCycle = zinfo->globPhaseCycles + zinfo->phaseLength;
	    }
//	    }
//		    
