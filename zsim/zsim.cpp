...
KNOB<INT32> KnobProcIdx(KNOB_MODE_WRITEONCE,
	"pintool",
	"procIdx",
	"0",
	"zsim process idx (internal)");

KNOB<INT32> KnobShmid(KNOB_MODE_WRITEONCE,
	"pintool",
	"shmid",
	"0",
...
KNOB<bool> KnobLogToFile(KNOB_MODE_WRITEONCE,
	"pintool",
	"logToFile",
	"false",
	"true if all messages should be logged to a logfile instead of stdout/err");

KNOB<string> KnobOutputDir(KNOB_MODE_WRITEONCE,
	"pintool",
	"outputDir",
	"./",
...
VOID Trace(TRACE trace, VOID *v) {
	if (!procTreeNode->isInFastForward() || !zinfo->ffReinstrument) {
		...
		for(BBL bbl = TRACE_BblHead(trace) ; BBL_Valid(bbl) ; bbl = BBL_Next(bbl)) {
			...
			BBL_InsertCall(bbl,
				IPOINT_BEFORE /*could do IPOINT_ANYWHERE if we redid load and store simulation in OOO*/,
...
VOID SyscallEnter(THREADID tid, CONTEXT *ctxt, SYSCALL_STANDARD std, VOID *v) {
	...
	if (fPtrs[tid].type != FPTR_JOIN && !zinfo->blockingSyscalls) {
		...
		zinfo->sched->syscallLeave(procIdx,
			tid,
			cid,
...
VOID SimEnd() {
	if (__sync_bool_compare_and_swap(&perProcessEndFlag, 0, 1) == false) {
		 //failed, note DEPENDS ON STRONG CAS
	    while(true) {
			 //sleep until thread that won exits for us
	        struct timespec tm;
			tm.tv_sec = 1;
			tm.tv_nsec = 0;
...
#define ZSIM_MAGIC_OP_PIM_SWITCH_TO         (1034)
#define ZSIM_MAGIC_OP_PIM_SWITCH_BACK         (1035)
...
VOID HandleMagicOp(THREADID tid, ADDRINT op) {
	...
	case ZSIM_MAGIC_OP_HEARTBEAT:
		...
		case 1030 :
		{
			...
			//	std::string cd = "\n";
				std::cout << "ALL.SIZE = " << zinfo->hmc.dis.size()<<std::endl;
			uint64_t max = 0;
			uint64_t maxload_dis = 0, maxstore_dis = 0,maxtotal_dis=0;
			uint64_t minload_dis = 0xFFFFFFFFFFFFFFFF, minstore_dis = 0xFFFFFFFFFFFFFFFF, mintotal_dis = 0xFFFFFFFFFFFFFFFF;
			double avg_load = 0, avg_store = 0, avg_dis = 0;
			uint64_t total_load = 0, total_store = 0;
			uint64_t count_dis = 0;
			//auto it = zinfo->hmc.dis.begin();
			//while (it != zinfo->hmc.dis.end()) {
			for(auto &it : zinfo->hmc.dis) {
				count_dis++;
			//	if (count_dis%5000==0)
			//	if (count_dis == zinfo->hmc.dis.size())
				//	std::cout << "FINI: = " << count_dis<<std::endl;
				//	break;
				if (it.first < min)
					min = it.first;
				if (it.first > max)
					max = it.first;
				//if(it.second.lastload)
				total_load += it.second.loadcount;
				total_store += it.second.storecount;
				if (it.second.loadcount!=0)
					avg_load += 1.0*it.second.total_load / it.second.loadcount;
				if (it.second.storecount != 0)
					avg_store += 1.0*it.second.total_store / it.second.storecount;
				if(it.second.loadcount + it.second.storecount>0)
					avg_dis += 1.0*it.second.total_dis / (it.second.loadcount + it.second.storecount);
				if (it.second.maxload > maxload_dis)
					maxload_dis = it.second.maxload;
				if (it.second.maxstore > maxstore_dis)
					maxstore_dis = it.second.maxstore;
				if (it.second.minload < minload_dis && it.second.minload!=0)
					minload_dis = it.second.minload;
				if (it.second.minstore < minstore_dis&&it.second.minstore!=0)
					minstore_dis = it.second.minstore;
				
				if (it.second.maxtotal > maxtotal_dis)
					maxtotal_dis = it.second.maxtotal;
				if (it.second.mintotal < mintotal_dis && it.second.mintotal != 0)
					mintotal_dis = it.second.mintotal;
			
			}
			std::cout << "TOTAL.AVG.DIS = " + std::to_string(avg_dis / count_dis) << std::endl;
			std::cout << "TOTAL.MAX.DIS = " + std::to_string(maxtotal_dis) << std::endl;
			std::cout << "TOTAL.MIN.DIS = " + std::to_string(mintotal_dis) << std::endl;
			
			std::cout << "AVG.LOAD_DIS = " + std::to_string(avg_load / count_dis) << std::endl;
			std::cout << "MAX.LOAD_DIS = " + std::to_string(maxload_dis) << std::endl;
			std::cout << "MIN.LOAD_DIS = " + std::to_string(minload_dis) << std::endl;
			std::cout << "AVG.STORE_DIS = " + std::to_string(total_store / count_dis) << std::endl;
			std::cout << "MAX.STORE_DIS = " + std::to_string(maxstore_dis) << std::endl;
			std::cout << "MIN.STORE_DIS = " + std::to_string(minstore_dis) << std::endl;
			std::cout << "DIS.MAX = " + std::to_string(max) << std::endl;
			std::cout << "DIS.MIN = " + std::to_string(min) << std::endl;
			std::cout << "PROG.SIZE = " + std::to_string((max - min) << 6) << std::endl;
			std::cout << "ACC.LOAD = " + std::to_string(total_load) << std::endl;
			std::cout << "ACC.STORE = " + std::to_string(total_store) << std::endl;
			std::cout << "ACC = " + std::to_string(total_load + total_store) << std::endl;
		...
		case 1033 :
		    return;
	case ZSIM_MAGIC_OP_PIM_SWITCH_TO:
		//fPtrs[tid] = zinfo->cores[1]->GetFuncPtrs
		if(zinfo->pim) {
			zinfo->switchmode = true;
		
			//setCid(0, 1);
			cores[tid] = zinfo->cores[1];
			zinfo->cores[1]->join();
			
			
		}else {
			//warn("ignore pim command in non-pim mode.");
		}
		zinfo->record.clear();
		return;
	case ZSIM_MAGIC_OP_PIM_SWITCH_BACK:
		if(zinfo->pim) {
			if (zinfo->record.size() > 0) {
				uint64_t pim_addr = (uint64_t)(std::get<1>(zinfo->record[zinfo->record.size() - 1]));
			
			
			//assert(std::to_string(std::get<2>(zinfo->record[zinfo->record.size() - 1])) == "1");
			zinfo->hmc.setCurrent(pim_addr);
			zinfo->hmc.PIM(pim_addr);
			//string s = "";
			for (uint i = 0; i < zinfo->record.size()-1; i++) {
				//	s += std::to_string(std::get<0>(zinfo->record[i]));
				//	s += " ";
				//	s += std::to_string(std::get<1>(zinfo->record[i]));
				//	s += " ";
				//	s += std::to_string(std::get<2>(zinfo->record[i]));
				//	s += " ";
				
				uint64_t need_addr = (uint64_t)(std::get<1>(zinfo->record[i]));
				if (need_addr > (0x7FFF00000000))
					continue;
				zinfo->hmc.PIMaccess(need_addr, std::get<2>(zinfo->record[i]));
				zinfo->hmc.PIMNeed(need_addr);
			}
			zinfo->hmc.PIMaccess(pim_addr, std::get<2>(zinfo->record[zinfo->record.size() - 1]));
				zinfo->record.clear();
				zinfo->hmc.resetCurrent();
			}
			
			zinfo->switchmode = false;
		//fPtrs[tid] = zinfo->cores[0]->GetFuncPtrs();
		cores[tid] = zinfo->cores[0];
		zinfo->cores[0]->join();
...
int main(int argc, char *argv[]) {
	...
	if (procIdx == 0 && !gm_isready()) {
		  // process 0 can exec() without fork()ing first, so we must check gm_isready() to ensure we don't initialize twice
	    masterProcess = true;
