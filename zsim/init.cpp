...
MemObject* BuildMemoryController(Config& config, uint32_t lineSize, uint32_t frequency, uint32_t domain, g_string& name) {
	...
	if (!config.get<bool>("sys.mem.pim", false))
	...
	{	
		...
		if (type == "Simple")
		{
			mem = new SimplePIMMemory(latency, name);
		}
		else
		{
...
static void InitSystem(Config& config) {
	zinfo->pim = config.get<bool>("sim.pim", false);
	...
	if (!zinfo->pim)
	{
		...
		vector<const char*>::iterator itor;
		for (itor = cacheGroupNames.begin(); itor != cacheGroupNames.end();)
		{
			string group(*itor);
			if (group == "pl1i" || group == "pl2" || group == "pl1d") 
			{
				// itor2=itor;
				itor = cacheGroupNames.erase(itor);
			}
			else
			{
				itor++;
	...
	if (zinfo->pim)
	{
		cMap["pl1d"] = BuildCacheGroup(config, "pl1d", true);
		cMap["pl1i"] = BuildCacheGroup(config, "pl1i", true);
	}
	...
	{
		childId = 0;
		for (BaseCache* llcBank : (*cMap["pl1d"])[0]) {
			llcBank->setParents(childId++, mems, network);
		}
	
		childId = 0;
		for (BaseCache* llcBank : (*cMap["pl1i"])[0]) {
			llcBank->setParents(childId++, mems, network);
		}
	}
    // Rest of caches
    ...
    if (!zinfo->traceDriven) {
	    ...
	    if (!zinfo->pim)
	    {
		    ...
		    vector<const char*>::iterator itor;
		    for (itor = coreGroupNames.begin(); itor != coreGroupNames.end();)
		    {
			    string group(*itor);
			    string prefix = string("sys.cores.") + group + ".";
			    
			    if (config.get<bool>(prefix + "pim", false)) 
			    {
				    // itor2=itor;
				    itor = coreGroupNames.erase(itor);
				    zinfo->numCores--;
			    }
			    else
			    {
	...
	if (zinfo->pim) {
		if (SimpleCore* core = dynamic_cast <SimpleCore*>(zinfo->cores[0])) {
			core->l2 = static_cast<Cache*>((*cMap["l2"])[0][0]);
			core->l3 = static_cast<Cache*>((*cMap["l3"])[0][0]);
		}
		else {
			panic("2");
		}
