...
bool Cache::tryInvalidate(const InvReq& req)
{
	int32_t lineId = array->lookup(req.lineAddr, nullptr, false);
	return lineId == -1 ? false : true;
}


uint64_t Cache::flush(const InvReq& req) {
	int32_t lineId = array->lookup(req.lineAddr, nullptr, false);
	if (MESIBottomCC* bcc = dynamic_cast<MESIBottomCC*>(cc))
	{
		MESIState* state = &(bcc->array)[lineId];
		if(*state != I);
		*state = I;
	}
	else
	{
		if (MESITerminalCC* tcc = dynamic_cast<MESITerminalCC*>(cc))
		{
			MESIState* state = &(tcc->bcc->array)[lineId];
			if(*state != I);
			*state = I;
		//	warn("not a MESIBottomCC");
			
		}
		else
		{
			if (MESITopCC* pcc = dynamic_cast<MESITopCC*>(cc))
			{
				bool r = true;
				pcc->sendInvalidates(req.lineAddr, lineId, req.type, &r, 0, req.srcId);
				//warn("not a MESIBottomCC");
			
			}
			else
			{
				if (MESICC* mcc = dynamic_cast<MESICC*>(cc))
				{
					bool rd = true;
					mcc->tcc->sendInvalidates(req.lineAddr, lineId, req.type, &rd, 0, req.srcId);
					MESIState* state = &(mcc ->bcc->array)[lineId];
					if(*state != I);
					*state = I;
